...
- 소스: **공식 사이트에서만 수집** — 기관(식약처 등)·제조사·쇼핑몰의 공식 웹사이트. 블로그·커뮤니티 등 비공식 출처 제외
...
- [x] **7. 임의 URL 추적 등록** — 상품 링크 붙여넣기 → 도메인별 파서 자동
      선택(렌블링/렌시스는 기존 파서, 그 외 generic: JSON-LD availability →
      품절 키워드 스캔 폴백) → 현재 판정 미리보기로 확인 후 등록 →
      크론 체크·⭐ 알림에 자동 합류
- [x] **8. 재고 수량 추적 + 예측 통계** — 렌시스 OMS API의 무도수 재고
      **수량**을 매 스윕 기록(stock_levels) → 소진 속도 계산 →
      "예상 품절일" 표시, 통계 탭에 수량 추이 그래프 추가.
      (무료 티어 보호: 수량이 변한 경우에만 기록)
- [ ] **(선택) 9. 제출물 정리** — README 아키텍처 다이어그램, 데모 시나리오
